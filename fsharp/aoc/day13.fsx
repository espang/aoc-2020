...
let firstAfter busId earliestTime =
    if earliestTime % busId = 0 then earliestTime else ((earliestTime / busId) + 1) * busId

let scoreForPart1 earliestTime (busId, departAt) = busId * (departAt - earliestTime)

(Seq.map
    (int >> (fun x -> x, firstAfter x earliestTime))
