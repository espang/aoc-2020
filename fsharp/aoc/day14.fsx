...
let numberToDecimalString (x:int) =
    System.Convert.ToString(int64 x, 2)    

let numberExtend l (s:string) =
    if s.Length < l
    then    
        String.replicate (l - s.Length) "0"
        + s
    else s
...
let lines =
    File.ReadAllLines "input_14.txt"
...
let parseLine (line:string) =
    ...
    then
        ...
        let v = (number.Match items.[0]).Groups.[1].Value
