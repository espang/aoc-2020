...
let numbers =
    Seq.map int (File.ReadAllLines "input_1.txt")
    |> Seq.toList
...
let findPair numbers sum =
    let rec _findPair acc numbers =
        match numbers with
        | head :: tail ->
            if Set.contains head acc
            then Some head
            else _findPair (Set.add (sum - head) acc) tail
        | [] -> None
...
let findTriple numbers sum =
    ...
    let rec _findTriple numbers =
        ...
        | head :: tail ->
            let pair =
                findPair (Set.toList (Set.remove head snumbers)) (sum - head)

            match pair with
            | Some result ->
                let third = sum - head - result
                Some(result * head * third)
            | None -> _findTriple tail
        | [] -> None
